...
if (!function_exists('mix')) {
    ...
    function mix($path)
    {
        ...
        $mixHotFile = kirby()->root('index') . $assetsDirectory . '/hot';
        if (F::exists($mixHotFile)) {
            // Remove white space from string and remove ending forward slash
            $mixHotFile = preg_replace('/\s+/', '', F::read($mixHotFile));
            $mixHotFile = substr($mixHotFile, 0, -1);
            $mixFilePath = $mixHotFile . $manifest[$path];
        } else {
        ...
        } else if (Str::contains($pathExtension, 'js')) {
